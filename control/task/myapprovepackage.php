...
<div >
   <form name="assignPackage" action="assignapprovepackage.php" method="post">
...
function openPackage(id){
	location = '../word/approveword.php?packageid=' + id;
}
...
$(function () {
	...
	$("#btnAssignPackage").click(function(){
		...
		var fields = {"categoryId":category.options[category.selectedIndex].value, 
				...
				"itemCount": itemCount}; 
		jQuery.post("assignapprovepackage.php", fields, function (data) {
			//var data = eval("(" + data + ")");
			console.log(data);
	...
	function getWordCount(){
		...
		jQuery.post("../word/_wordcontroller.php?op=approvecount", {"WordCategory": $("#categories").val()}, function (data) {
			var count = eval("(" + data + ")");
	...
	var opts = {//"getCountUrl":"_get_editpackage_count.php",
			"getDataUrl":"_get_approvepackage_pagination.php",
			"target":".ypager",
			"pageSize":30,
			...
			"onFill":function(rows){
				...
				for (var i = 0; i < rows.length; i++) {
			        ...
			        + row.PackageId + "'><div class='bar' style='width: " + row.Percent + "%;display:inline;'>" + row.Percent + "%</div></div></td> <td style='width:90px;'><a href='../word/approveword.php?packageid=" 
			        + rows[i].PackageId + "' style='margin-left:10px;' title='详情' target='_blank'>详情</a>";
